...
# Input CSVs are generated by scripts/benchmark.sh with OUT_DIR split by decomposition.
INPUTS = {
...
def summarize_strong(rows: list[dict[str, str]], decomposition: str) -> list[dict[str, object]]:
    # Use rank=1 as baseline for speedup/efficiency.
...
def summarize_weak(rows: list[dict[str, str]], decomposition: str) -> list[dict[str, object]]:
    # Relative weak-scaling time compares each run to the single-rank baseline.
    baseline_total = as_float(rows[0], "total_seconds")
    summary_rows = []
    for row in rows:
        total = as_float(row, "total_seconds")
...
def main() -> None:
    # Produce both machine-readable summaries and a markdown snapshot for report writing.
