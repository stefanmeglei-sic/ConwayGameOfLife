...
def split_by_decomposition(rows):
    # Keep deterministic plotting order across runs.
    buckets = {"1d": [], "2d": []}
    for row in rows:
...
def line_chart_svg(title, x_label, y_label, x_values, series, y_min=None, y_max=None):
    # Lightweight SVG renderer so plots work without matplotlib.
    width = 920
    height = 560
    left = 70
    right = 30
...
def main():
    # Read summary CSVs generated by summarize_benchmarks.py.
